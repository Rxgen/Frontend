...
export async function fetchProducts(letter = "", page = 1,category = "",brand="", productName="", ndc = "",pageSize = 12, ) {
  ...
  if (category === "NEW_PRODUCTS") {
    ...
    url = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/products?sort[0]=createdAt:desc&populate[0]=product_images.slide&populate[1]=pdf_files.pdf&pagination[page]=${page}&pagination[pageSize]=${pageSize}`;
  } else if (ndc) {
    console.log("Case: NDC", ndc);
    url = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/products?filters[ndc][$contains]=${ndc}&populate[0]=product_images.slide&populate[1]=pdf_files.pdf&pagination[page]=${page}&pagination[pageSize]=${pageSize}`;
  } else if (brand) {
    console.log("Case: BRAND", brand);
    url = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/products?filters[brand][name][$startsWith]=${brand}&populate[0]=product_images.slide&populate[1]=pdf_files.pdf&pagination[page]=${page}&pagination[pageSize]=${pageSize}`;
  } else if (productName) {
    console.log("Case: PRODUCT_NAME", productName);
    url = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/products?filters[product_name][$startsWith]=${productName}&populate[0]=product_images.slide&populate[1]=pdf_files.pdf&pagination[page]=${page}&pagination[pageSize]=${pageSize}`;
  } else if (letter) {
    console.log("Case: LETTER", letter);
    url = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/products?filters[product_name][$startsWith]=${letter}&populate[0]=product_images.slide&populate[1]=pdf_files.pdf&pagination[page]=${page}&pagination[pageSize]=${pageSize}`;
  } else {
